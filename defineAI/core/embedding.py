...
class FolderIndex:
    ...
    @classmethod
    def from_files(
        cls, files: List[File], embeddings: Embeddings, vector_store: Any
    ) -> "FolderIndex":
        """
...
def delete_all_embeddings(vector_store: VectorStore) -> None:
    """
    Delete all embeddings in the vector store.
    """
    data = vector_store._collection.get()  # type: ignore
    if data is not None and "ids" in data:
        batch_size = 166
        for i in range(0, len(data["ids"]), batch_size):
            batch_ids = data["ids"][i : i + batch_size]
            vector_store._collection.delete(ids=batch_ids)  # type: ignore


def show_loaded_documents(vector_store: VectorStore) -> None:
    """
    Show all loaded documents in the vector store.
    """
    data = vector_store._collection.get()  # type: ignore
    file_names = set(metadata["file_name"] for metadata in data.get("metadatas", []))
    ...
    try:
        if file_names:
            ...
            st.write("No documents are loaded.")
    except Exception as e:
        st.error("Error: try to delete data and reload.")
